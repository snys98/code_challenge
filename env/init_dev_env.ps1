...
$cert = Get-CCertificate -FriendlyName "dev.challenge.io" -StoreLocation LocalMachine -StoreName Root
$pwd = ConvertTo-SecureString -String "dev.challenge.io" -AsPlainText -Force
if ($null -eq $cert) {
  $cert = New-SelfSignedCertificate -CertStoreLocation Cert:\LocalMachine\My -Subject dev.challenge.io -DnsName dev.challenge.io, *.api.dev.challenge.io, *.dev.challenge.io -FriendlyName "dev.challenge.io" -NotAfter (Get-Date).AddYears(1000)
  mkdir ./certs -ErrorAction Ignore
  Export-PfxCertificate -Cert "Cert:\LocalMachine\My\$($cert.Thumbprint)" -FilePath "./certs/dev.challenge.io.pfx" -Password $pwd
  openssl pkcs12 -in "./certs/dev.challenge.io.pfx" -nodes -out ./certs/dev.challenge.io.pem -passin pass:dev.challenge.io
  openssl pkcs12 -in "./certs/dev.challenge.io.pfx" -out ./certs/dev.challenge.io.crt -clcerts -nokeys -passin pass:dev.challenge.io
  openssl pkcs12 -in "./certs/dev.challenge.io.pfx" -out ./certs/dev.challenge.io.key -nocerts -nodes -passin pass:dev.challenge.io
  Install-CCertificate -Path "./certs/dev.challenge.io.pfx" -StoreLocation LocalMachine -StoreName Root -Password $pwd
}
